...
const inputCls = 'w-full bg-input-bg border border-border-color rounded px-2.5 py-1.5 text-xs text-text-main focus:outline-none focus:border-brand-accent';
    ...
    <aside className="w-[380px] border-r border-border-color/60 bg-sidebar flex flex-col flex-shrink-0 overflow-y-auto p-5 space-y-5">
      ...
      <div className="space-y-3">
        ...
        <div className="border border-border-color/50 rounded-xl overflow-hidden bg-card/10">
          <SectionHeader id="target" icon={Briefcase} label="Target Position" openSection={openSection} onToggle={toggle} />
          <AnimatePresence initial={false}>
            {openSection === 'target' && (
              <motion.div key="target" {...SECTION_ANIM} style={{ overflow: 'hidden' }}>
        ...
        {/* ── Letter Paragraphs ─────────────────────────────────── */}
        <div className="border border-border-color/50 rounded-xl overflow-hidden bg-card/10">
          <SectionHeader id="paragraphs" icon={AlignLeft} label="Letter Paragraphs" openSection={openSection} onToggle={toggle} />
          <AnimatePresence initial={false}>
            {openSection === 'paragraphs' && (
              <motion.div key="paragraphs" {...SECTION_ANIM} style={{ overflow: 'hidden' }}>
        ...
        {/* ── Highlights ────────────────────────────────────────── */}
        <div className="border border-border-color/50 rounded-xl overflow-hidden bg-card/10">
          <SectionHeader id="highlights" icon={FileText} label="Highlights" badge={state.highlights.length} openSection={openSection} onToggle={toggle} />
          <AnimatePresence initial={false}>
            {openSection === 'highlights' && (
              <motion.div key="highlights" {...SECTION_ANIM} style={{ overflow: 'hidden' }}>
                <div className="p-4 border-t border-border-color/40 space-y-3">
