...
${jobDesc}`;
    ...
    return this.request<Partial<ResumeState>>(apiKey, payload);
  }

  // Resume PDF Parsing
  public static async parseResumePdf(apiKey: string, base64Data: string): Promise<Partial<ResumeState>> {
    const prompt = "Parse this resume PDF and extract the details into the requested JSON schema. Make sure to capture contact details, professional summary, skills list, work experiences, education, certifications, key achievements, and languages.";
    ...
    const payload = {
      ...
      generationConfig: {
        ...
        responseSchema: {
          ...
          required: ["name", "subtitle", "phone", "email", "linkedin", "location", "resumeSummary", "resumeSkills", "resumeExperience", "resumeEducation", "resumeCerts", "resumeAchievements", "resumeLanguages"]
        }
      }
    };
  ...
  // Inject Single Keyword into Cover Letter
...
3. Use double asterisks to bold the keyword (e.g. **${keyword}**) where you inject it.
...
${JSON.stringify({
  ...
  // Inject Single Keyword into Resume
...
${jdText ? `TARGET JOB DESCRIPTION:\n${jdText}` : ""}`;
    ...
    const payload = {
      ...
      generationConfig: {
        ...
        responseSchema: {
          type: "OBJECT",
          properties: {
            improvedBullet: { type: "STRING", description: "The refined, professional, and impact-focused bullet points." }
          },
    ...
    const res = await this.request<ImprovedResponse>(apiKey, payload);
    return res.improvedBullet;
  }

  // Scraper Text Extraction Fallback
...
${rawHtml.substring(0, 15000)}`;
    ...
    const payload = {
      ...
      generationConfig: {
        ...
        responseSchema: {
          type: "OBJECT",
          properties: {
            extractedJd: { type: "STRING", description: "The extracted clean text of the job description." }
          },
    ...
    const res = await this.request<ExtractedResponse>(apiKey, payload);
