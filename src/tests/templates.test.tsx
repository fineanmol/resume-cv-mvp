...
describe('ResumeTemplates — smoke: all templates render', () => {
  ALL_TEMPLATES.forEach(template => {
    it(`${template} renders .pdf-sheet without crashing`, async () => {
      const { container } = await renderResumeTemplate({ state: resumeWith({ template }) });
...
describe('ResumeTemplates — candidate name shown in every template', () => {
  ALL_TEMPLATES.forEach(template => {
    it(`${template} shows "Jonathan Doe"`, async () => {
      const { getAllByText } = await renderResumeTemplate({ state: resumeWith({ template }) });
...
describe('ResumeTemplates — body fontFamily applied to .pdf-sheet', () => {
  (['inter', 'outfit', 'eb-garamond', 'jetbrains-mono'] as const).forEach(font => {
    it(`fontFamily="${font}" → .pdf-sheet.style.fontFamily contains font name`, async () => {
      const { container } = await renderResumeTemplate({ state: resumeWith({ template: 'navy', fontFamily: font }) });
...
describe('ResumeTemplates — headingFont applied to name heading in all templates', () => {
  ALL_TEMPLATES.forEach(template => {
    it(`${template}: headingFont=eb-garamond → EB Garamond in DOM`, async () => {
      const { container } = await renderResumeTemplate({ state: resumeWith({ template, headingFont: 'eb-garamond' }) });
...
describe('ResumeTemplates — brandColor appears in DOM for all templates', () => {
  ...
  ALL_TEMPLATES.forEach(template => {
    it(`${template}: brandColor #cc2200 appears in at least one element style`, async () => {
      const { container } = await renderResumeTemplate({ state: resumeWith({ template, brandColor }) });
...
describe('ResumeTemplates — showPhoto toggle', () => {
  ...
  photoTemplates.forEach(template => {
    it(`${template}: profile img present when showPhoto=true`, async () => {
      const { container } = await renderResumeTemplate({ state: { ...withPhoto, layoutSettings: { ...withPhoto.layoutSettings, template, showPhoto: true } } });
      expect(container.querySelector('img[alt="Jonathan Doe"]')).toBeTruthy();
    });

    it(`${template}: profile img absent when showPhoto=false`, async () => {
      const { container } = await renderResumeTemplate({ state: { ...withPhoto, layoutSettings: { ...withPhoto.layoutSettings, template, showPhoto: false } } });
      expect(container.querySelector('img[alt="Jonathan Doe"]')).toBeNull();
    });
  });

  // Navy headerStyle variants that support photo
  it('navy banner: img present when showPhoto=true', async () => {
    const { container } = await renderResumeTemplate({ state: { ...withPhoto, layoutSettings: { ...withPhoto.layoutSettings, template: 'navy', headerStyle: 'banner', showPhoto: true } } });
    expect(container.querySelector('img')).toBeTruthy();
  });

  it('navy banner: img absent when showPhoto=false', async () => {
    const { container } = await renderResumeTemplate({ state: { ...withPhoto, layoutSettings: { ...withPhoto.layoutSettings, template: 'navy', headerStyle: 'banner', showPhoto: false } } });
...
describe('ResumeTemplates — contact field visibility via layoutSettings', () => {
  ...
  (['sidebar', 'designer'] as const).forEach(template => {
    it(`${template}: hides phone when showPhone=false`, async () => {
      const { queryByText } = await renderResumeTemplate({ state: {
          ...contactState,
    ...
    it(`${template}: hides title when showTitle=false`, async () => {
...
describe('ResumeTemplates — navy headerStyle variants render', () => {
  (['centered', 'left', 'banner', 'minimal'] as const).forEach(style => {
    it(`headerStyle="${style}" renders without crashing`, async () => {
      const { container } = await renderResumeTemplate({ state: resumeWith({ template: 'navy', headerStyle: style }) });
      expect(container.querySelector('.pdf-sheet')).toBeTruthy();
    });
  });
});
...
describe('ResumeTemplates — content sections render', () => {
  it('navy: skills tags visible', async () => {
    const { getByText } = await renderResumeTemplate({ state: resumeWith({ template: 'navy' }) });
    expect(getByText('React')).toBeTruthy();
  });

  it('navy: experience company name visible', async () => {
    const { getByText } = await renderResumeTemplate({ state: resumeWith({ template: 'navy' }) });
    expect(getByText('TechFlow Solutions')).toBeTruthy();
  });

  it('navy: certifications visible', async () => {
    const { getAllByText } = await renderResumeTemplate({ state: resumeWith({ template: 'navy' }) });
    expect(getAllByText(/AWS Certified/i).length).toBeGreaterThan(0);
  });

  it('sidebar: languages in left column', async () => {
    const { getByText } = await renderResumeTemplate({ state: resumeWith({ template: 'sidebar' }) });
    expect(getByText('English')).toBeTruthy();
  });

  it('tech: achievements section renders', async () => {
    const { getAllByText } = await renderResumeTemplate({ state: resumeWith({ template: 'tech' }) });
...
describe('ResumeTemplates — bulletStyle customization', () => {
  it('renders arrow bullet points in navy when bulletStyle="arrow"', async () => {
    const { container } = await renderResumeTemplate({ state: resumeWith({ template: 'navy', bulletStyle: 'arrow' }) });
    expect(container.textContent).toContain('➤');
  });

  it('renders number bullet points in navy when bulletStyle="number"', async () => {
    const { container } = await renderResumeTemplate({ state: resumeWith({ template: 'navy', bulletStyle: 'number' }) });
    expect(container.textContent).toContain('1.');
    expect(container.textContent).toContain('2.');
  });

  it('does not render bullet markers when bulletStyle="none"', async () => {
    const { container } = await renderResumeTemplate({ state: resumeWith({ template: 'navy', bulletStyle: 'none' }) });
...
describe('ResumeTemplates — work and certification links', () => {
  it('renders experience work link with correct href and data-href', async () => {
    const { container } = await renderResumeTemplate({ state: DEFAULT_RESUME_STATE });
  ...
  it('renders certification link with correct href and data-href', async () => {
    const { container } = await renderResumeTemplate({ state: DEFAULT_RESUME_STATE });
...
describe('ResumeTemplates — skillsStyle option', () => {
  it('renders skills as chips by default', async () => {
    const { container } = await renderResumeTemplate({ state: resumeWith({ template: 'navy', skillsStyle: 'chips' }) });
    const chip = container.querySelector('span.inline-flex.w-max');
    expect(chip).toBeTruthy();
    expect(chip?.textContent).toBe('React');
  });

  it('renders skills as normal text when skillsStyle="normal"', async () => {
    const { container } = await renderResumeTemplate({ state: resumeWith({ template: 'navy', skillsStyle: 'normal' }) });
    const chip = container.querySelector('span.inline-flex.w-max');
...
describe('ResumeTemplates — BottomSections entry visibility', () => {
  it('navy: hides cert link when entry visibility.link=false', async () => {
    ...
    const { container } = await renderResumeTemplate({ state: state });
    expect(container.querySelector('a[href="https://example.com/cert"]')).toBeNull();
  });

  it('navy: hides language level when entry visibility.level=false', async () => {
    const state: ResumeState = {
      ...DEFAULT_RESUME_STATE,
      layoutSettings: { ...DEFAULT_RESUME_STATE.layoutSettings, template: 'navy' },
  ...
  it('navy: cert multiline desc renders as bullet list', async () => {
    const state: ResumeState = {
      ...DEFAULT_RESUME_STATE,
...
describe('ResumeTemplates — editable integration', () => {
  ...
  it('navy: BottomSections Add entry buttons exist when isEditable', async () => {
  ...
  it('serif: SectionWrapper edit toolbar present when isEditable', async () => {
    const { container } = await renderResumeTemplate({ state: resumeWith({ template: 'serif' }), isEditable: true });
    expect(container.querySelectorAll('.edit-only').length).toBeGreaterThan(0);
  });

  it('designer: draggable sections render when showLayoutBounds is true', async () => {
    const { container } = await renderResumeTemplate({
  ...
  it('designer: custom column order renders sections in configured columns', async () => {
    const { getByText, queryByText } = await renderResumeTemplate({
      state: resumeWith({
        template: 'designer',
...
describe('ResumeTemplates — header settings toolbar', () => {
  ...
  it('navy template shows header settings gear when editable', async () => {
    const { container } = await renderResumeTemplate({ state: resumeWith({ template: 'navy' }), isEditable: true });
    expect(container.querySelector('header [title="Header Settings"]')).toBeTruthy();
  });
});
...
describe('ResumeTemplates — design panel spacing settings applied to sheet', () => {
  it('fontSize sets pdf-sheet fontSize and --sheet-fs variable', async () => {
    const { container } = await renderResumeTemplate({ state: resumeWith({ template: 'navy', fontSize: 12 }) });
    const sheet = container.querySelector('.pdf-sheet') as HTMLElement;
    expect(sheet.style.fontSize).toBe('12pt');
    expect(sheet.style.getPropertyValue('--sheet-fs')).toBe('12');
  });

  it('lineHeight sets pdf-sheet lineHeight', async () => {
    const { container } = await renderResumeTemplate({ state: resumeWith({ template: 'navy', lineHeight: 1.65 }) });
    const sheet = container.querySelector('.pdf-sheet') as HTMLElement;
    expect(sheet.style.lineHeight).toBe('1.65');
  });

  it('padding sets pdf-sheet padding in mm', async () => {
    const { container } = await renderResumeTemplate({ state: resumeWith({ template: 'navy', paddingTopBottom: 20, paddingLeftRight: 18 }) });
    const sheet = container.querySelector('.pdf-sheet') as HTMLElement;
    expect(sheet.style.padding).toBe('20mm 18mm');
  });

  it('sectionSpacing sets marginBottom on section blocks', async () => {
    const { container } = await renderResumeTemplate({ state: resumeWith({ template: 'navy', sectionSpacing: 22 }) });
    const section = container.querySelector('section') as HTMLElement;
    expect(section?.style.marginBottom).toBe('22px');
  });

  it('columnGap sets gap on designer column grid', async () => {
    const { getByTestId } = await renderResumeTemplate({ state: resumeWith({ template: 'designer', columnGap: 28 }) });
